...
public class TreeMafiaOrganization implements MafiaOrganization {
	...
	public void reinstateMafioso(Mafioso boss, Mafioso mafioso, List<Mafioso> directSubordinates) {
		if (boss != null) {
			...
			// I'm reinstating the Cupula. In this case I need to make sure that the
			// MafiaCell is properly configured
	...
	private MafiaCell promoteOldestOf(List<MafiaCell> siblings) {
		...
		if (oldest != null) {
			promoteOldest(siblings, oldest);
		}
		return oldest;
	}

	private void promoteOldest(List<MafiaCell> siblings, final MafiaCell oldest) {
		siblings.remove(oldest);
		siblings.forEach(new Consumer<MafiaCell>() {
			@Override
			public void accept(MafiaCell t) {
				oldest.addSubordinate(t.getMafioso());
			}
		});
